...
COQLIBS:= -R src Containers.Plugin\
  -R theories Containers
COQDOCLIBS:=-R src Containers.Plugin\
  -R theories Containers
...
VOFILES:=$(VFILES:.v=.vo)
VOFILES2:=$(patsubst theories/%,%,$(filter theories/%,$(VOFILES)))
VOFILES0:=$(filter-out src/% theories/%,$(VOFILES))
...
CMOFILES0:=$(filter-out src/% theories/%,$(CMOFILES))
CMIFILES:=$(MLFILES:.ml=.cmi)
CMIFILES1:=$(patsubst src/%,%,$(filter src/%,$(CMIFILES)))
CMIFILES0:=$(filter-out src/% theories/%,$(CMIFILES))
CMXFILES:=$(MLFILES:.ml=.cmx)
CMXSFILES:=$(MLFILES:.ml=.cmxs)
CMXSFILES1:=$(patsubst src/%,%,$(filter src/%,$(CMXSFILES)))
CMXSFILES0:=$(filter-out src/% theories/%,$(CMXSFILES))
OFILES:=$(MLFILES:.ml=.o)

all: $(VOFILES) $(CMOFILES) $(CMXSFILES) $(CONTAINERS_PLUGIN)\
...
install:
	...
	(cd theories; for i in $(VOFILES2); do \
	...
	(cd src; for i in $(CMOFILES1); do \
	 install -d `dirname $(COQLIB)/user-contrib/Containers/Plugin/$$i`; \
	 install $$i $(COQLIB)/user-contrib/Containers/Plugin/$$i; \
	 done)
...
clean:
	...
	- rm -rf html
	- rm -f $(CONTAINERS_PLUGIN)
	- rm -f $(CONTAINERS_PLUGINOPT)
	- rm -f 
